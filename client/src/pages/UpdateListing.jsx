...
export default function UpdateListing() {
  ...
  const [imageUploadError, setImageUploadError] = useState(false);    //  for error check in images
  const [uploading, setUploading] = useState(false)  // for loading 
  ...
  useEffect(() => {
    const fetchListing = async() => {
        ...
        // listingId used with the params is given in index.js
        const listingId = params.listingId;
        // this will print the id from the url
  ...
  const handleImageSubmit = () => {
    //  first we check if there is a file and it should be less than 7. max is 6
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      setUploading(true);
      setImageUploadError(false);
      // we gonna upload more than 1 image and it will more than one asynchronous behaviour. so there will be more than one promise
      const promises = [];
  ...
  const handleChange = (e) => {
    ...
    if (e.target.type === 'number' || e.target.type === 'text' || e.target.type === 'textarea') {
      setFormData({
        ...formData,
        [e.target.id]: e.target.value   // when [] is used it will return as name otherwise it will return 'name'
      })
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // these price is sometimes is number or string . so inorder to prevent put + 
          ...
          <p className="text-red-700">{imageUploadError && imageUploadError}</p>
          {
            formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
              <div key={index} className="flex justify-between p-3 border items-center">
                ...
                <button type="button" onClick={() => handleRemoveImage(index)} className="p-3 rounded-lg hover:opacity-75 text-red-700 uppercase">Delete</button>
              </div>
            ))
          }
